...
public class UsuarioController {
	...
	@ApiOperation(value = "Servicio consultar", notes = "Este servicio se encarga de consultar los usuarios existentes")
	@ApiResponses({ @ApiResponse(code = HttpServletResponse.SC_OK, message = "OK") })
	public ResponseEntity<List<Usuario>> consultar() throws IOException {
		List<Usuario> listaUsuarios = service.get();
	...
	@ApiResponses({ @ApiResponse(code = HttpServletResponse.SC_OK, message = "OK"),
			@ApiResponse(code = HttpServletResponse.SC_NOT_FOUND, message = "Este usuario no existe") })
	public ResponseEntity<Usuario> consultarPorId(@PathVariable @NotEmpty @NotNull int id) throws IOException {
	...
	@ApiResponses({ @ApiResponse(code = HttpServletResponse.SC_CREATED, message = "Se ha guardado con exito"),
			@ApiResponse(code = HttpServletResponse.SC_NOT_ACCEPTABLE, message = "Este usuario ya existe") })
	public ResponseEntity<String> guardar(@Valid @RequestBody Usuario usuario) throws Exception {
		String respuesta = service.save(usuario);
	...
	@ApiResponses({ @ApiResponse(code = HttpServletResponse.SC_OK, message = "Se ha editado exitosamente"),
			@ApiResponse(code = HttpServletResponse.SC_NOT_FOUND, message = "Este usuario no existe") })
	public ResponseEntity<String> editar(@Valid @RequestBody Usuario usuario) throws Exception {
		String respuesta = service.update(usuario);
